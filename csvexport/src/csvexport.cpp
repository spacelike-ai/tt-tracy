...
Args parse_args(int argc, char** argv)
{
    ...
    int c;
    while ((c = getopt_long(argc, argv, "hf:p:x:s:ceugmp", long_opts, NULL)) != -1)
    {
        switch (c)
        {
        case 'h':
...
int main(int argc, char** argv)
{
    ...
    {
        columns = {
            "name", "src_file", "src_line", "zone_name", "zone_text", "ns_since_start", "exec_time_ns", "thread", "special_parent_text", "value"
        };
    }
    else
    {
